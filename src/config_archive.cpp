...
namespace midimagic {
    ...
    const config_archive::operation_result config_archive::loadon() {
        ...
        // check version
        if (eeprom_buffered_read_byte(static_header_field::VERSION) != RUNNING_VERSION) {
            return operation_result::VERSION_UNKNOWN;
        }

        // get stored size
        u16 total_size = eeprom_buffered_read_byte(static_header_field::SIZE0) << 8;
        total_size += eeprom_buffered_read_byte(static_header_field::SIZE1);
        ...
        u8 portgroup_config_count = eeprom_buffered_read_byte(static_header_field::PORTGROUP_CONFIG_COUNT);
    ...
    const config_archive::operation_result config_archive::writeout() {
        ...
        eeprom_buffered_write_byte(static_header_field::SIZE0, (u8) (total_size >> 8));
        eeprom_buffered_write_byte(static_header_field::SIZE1, (u8) (total_size & 0xff));
    ...
    uint32_t config_archive::generate_archive_header() {
        // write magic
        eeprom_buffered_write_byte(static_header_field::MAGIC0, MAGIC);
        eeprom_buffered_write_byte(static_header_field::MAGIC1, MAGIC);
        // write version
        eeprom_buffered_write_byte(static_header_field::VERSION, RUNNING_VERSION);

        // write config counts
        u8 port_config_count = m_system_state.system_ports.size();
        u8 portgroup_config_count = m_system_state.system_port_groups.size();
        eeprom_buffered_write_byte(static_header_field::PORT_CONFIG_COUNT, port_config_count);
        eeprom_buffered_write_byte(static_header_field::PORTGROUP_CONFIG_COUNT, portgroup_config_count);
        ...
        for (u8 i = 0; i < port_config_count; i++) {
            eeprom_buffered_write_byte(running_header_field_addr, (u8) running_config_base_addr >> 24);
            eeprom_buffered_write_byte(running_header_field_addr + 1, (u8) ((running_config_base_addr >> 16) & 0xff));
            eeprom_buffered_write_byte(running_header_field_addr + 2, (u8) ((running_config_base_addr >> 8) & 0xff));
            eeprom_buffered_write_byte(running_header_field_addr + 3, (u8) (running_config_base_addr & 0xff));
    ...
    uint32_t config_archive::get_address_to(config_type type, u8 index) {
        ...
        out_addr += ((uint32_t) eeprom_buffered_read_byte(header_field_addr)) << 24;
        out_addr += ((uint32_t) eeprom_buffered_read_byte(header_field_addr + 1)) << 16;
        out_addr += ((uint32_t) eeprom_buffered_read_byte(header_field_addr + 2)) << 8;
        out_addr += ((uint32_t) eeprom_buffered_read_byte(header_field_addr + 3));
        return out_addr;
    }
    ...
    uint32_t config_archive::serialise(struct output_port_config config, uint32_t base_addr) {
        ...
        eeprom_buffered_write_byte(base_addr + v1_port_config_field::PORT_NUMBER, config.port_number);
        eeprom_buffered_write_byte(base_addr + v1_port_config_field::CLOCK_RATE, config.clock_rate);
        eeprom_buffered_write_byte(base_addr + v1_port_config_field::VELOCITY, config.velocity_output);
    ...
    uint32_t config_archive::serialise(struct port_group_config config, uint32_t base_addr) {
        ...
        eeprom_buffered_write_byte(base_addr + v1_portgroup_config_field::DEMUX_TYPE, config.demux);
        eeprom_buffered_write_byte(base_addr + v1_portgroup_config_field::MIDI_CHANNEL, config.midi_channel);
        eeprom_buffered_write_byte(base_addr + v1_portgroup_config_field::CC_NUMBER, config.cont_controller_number);
        if (config.transpose_offset < 0) {
            eeprom_buffered_write_byte(base_addr + v1_portgroup_config_field::TRANSPOSE, (u8) (0x80 | (config.transpose_offset & 0x7f)));
        } else {
            eeprom_buffered_write_byte(base_addr + v1_portgroup_config_field::TRANSPOSE, config.transpose_offset);
        }
        eeprom_buffered_write_byte(base_addr + v1_portgroup_config_field::INPUT_TYPE_COUNT, config.input_types.size());
        eeprom_buffered_write_byte(base_addr + v1_portgroup_config_field::OUTPUT_PORT_COUNT, config.output_port_numbers.size());

        for (auto &input_type: config.input_types) {
            eeprom_buffered_write_byte(base_addr + configuration_size, input_type);
            configuration_size++;
        }

        for (auto &port_number: config.output_port_numbers) {
            eeprom_buffered_write_byte(base_addr + configuration_size, port_number);
            configuration_size++;
        }
